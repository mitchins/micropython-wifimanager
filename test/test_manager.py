...
class LogicTests(unittest.TestCase):
    ...
    def test_fallback_choose_single(self):
        ...
        self.assertEqual(interface.DEBUG_CONNECTED_SSID, "HomeNetwork")
        self.assertEqual(interface.DEBUG_CONNECTED_BSSID, b'\x90r@\x1f\xf0\xe4')
        # Checks on th AP
    ...
    def test_fallback_ap(self):
        ...
        WifiManager.setup_network()
        # Checks on the client
        self.assertFalse(interface.isconnected())
        self.assertIsNone(interface.DEBUG_CONNECTED_SSID)
        self.assertIsNone(interface.DEBUG_CONNECTED_BSSID)
        # Checks on th AP
        self.assertTrue(host.active())
        self.assertEqual(host.config_dict['essid'], "Micropython-Dev")
    ...
    def test_never_ap(self):
        ...
        host = network.WLAN(network.AP_IF)
        interface.scan_results = sample_scans.scan3()
        WifiManager.config_file = 'test/networks_never.json'
        WifiManager.setup_network()
        # Checks on the client
        self.assertFalse(interface.isconnected())
        self.assertIsNone(interface.DEBUG_CONNECTED_SSID)
...
class AsyncTests(unittest.TestCase):
    ...
    def testStart(self):
        network.DEBUG_RESET()
        WifiManager.config_file = 'test/networks_fallback.json'
        self.assertTrue(WifiManager.start_managing())
